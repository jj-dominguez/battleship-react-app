...
// Renders individual row with labeled td depending on board state
    ...
    clickHandler(x,y){
        const { board , turns, totalShips, totalBattleships, totalSubmarines, totalDestroyers, totalPaddleboats } = this.state
        var newBoard = board;
        var turnsLeft = turns;
        ...
        if (this.state.turns === 0 || shipsLeft === 0) { // state: game is over
            ...
            if(shipNumber === 15){
                battleshipsLeft === 1 ? this.setState({BattleshipSunk: 'Battleship RIP'}) : battleshipsLeft--
            } else if (shipNumber === 13) {
                submarinesLeft === 1 ? this.setState({SubmarineSunk: 'Submarine RIP X2'}) : submarinesLeft--
            } else if (shipNumber === 12) {
                destroyersLeft === 1 ? this.setState({DestroyerSunk: 'Destroyer RIP X2'}) : destroyersLeft--
            } else if (shipNumber === 11) {
                paddleboatsLeft === 1 ? this.setState({PaddleboatSunk: 'Paddleboat RIP'}) : paddleboatsLeft--
            }
                ...
                */
                for(var i = 0; i < newBoard.length; i++){
                    for(var j = 0; j < newBoard[i].length; j++){
                        if (newBoard[i][j] === 11 || newBoard[i][j] === 12 ||
                        newBoard[i][j] === 13 || newBoard[i][j] === 15){
                            newBoard[i][j] = 2;
                        }
                    }
                }
